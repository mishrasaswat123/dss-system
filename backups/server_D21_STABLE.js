...
advisoryNarrative = `Current stance suggests ${advisory.stance.toLowerCase()} positioning. Investors may consider ${advisory.action.toLowerCase()}. ${advisory.allocationGuidance}. ${advisory.riskNote}.`;
...
app.get("/callback", (req, res) => {
  const session = req.query.apisession;

  if (!session) {
    return res.send("❌ No session received");
  }

  console.log("✅ Breeze Session Received:", session);

  const path = "/home/ubuntu/dss-system/breeze/session.txt";

  try {
    // Ensure directory exists
    fs.mkdirSync("/home/ubuntu/dss-system/breeze", { recursive: true });

    console.log("📁 Writing session to:", path);

    // Write session to file
    fs.writeFileSync(path, session);

    console.log("✅ Session saved at:", path);
  } catch (e) {
    console.log("❌ File write error:", e);
  }

  res.send("✅ Session captured successfully. You can close this.");
});
/* =========================
   API
========================= */
