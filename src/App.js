						...
						function Chip({ label, bull }) {
						  ...
						  return (
							<span style={{ ...style, padding:"0px 6px",
			borderRadius:2,
			fontSize:"0.54rem", fontWeight: 700, letterSpacing: "0.06em", fontFamily: "monospace" }}>
							  {label}
							</span>
						  );
						}
					...
					padding:12,
						...
						function KPICard({ label, value, sub, color }) {
						  return (
							<Panel style={{
						  borderLeft:`3px solid ${color || "#1e2d3d"}`,
						  padding:14,
						}}>
	...
	// ===============================
	// LIVE EQUITY SIGNALS STATE
	// ===============================

	const EMPTY_EQUITY_STATE = {
	...
	const EQUITY_API =
	  ...
	  let LIVE_EQUITY_CACHE =
	  EMPTY_EQUITY_STATE;

	async function fetchEquitySignalsData() {
	...
	async function fetchSectorRotationData() {
	  ...
	  try {
		...
		if (!res.ok) {
		  throw new Error("Sector API failed");
		}

		const json = await res.json();

		return json?.data || EMPTY_SECTOR_STATE;
		...
		return EMPTY_SECTOR_STATE;
						...
						function OverviewTab() {
						  ...
						  <table style={{ width: "100%", borderCollapse: "collapse" }}>
							...
							<tbody>
					  {[
						["Advance/Decline","0.72",false],
						["% Above 50 DMA","38%",false],
						["% Above 200 DMA","52%",null],
						["New 52W Highs","41",null],
						["New 52W Lows","87",false]
					  ].map(([n,v,b],i)=>(
						<tr key={i}>
						  <td style={{
							padding: "6px 8px",
	...
	const techSignals =
	  equityData.technicalSignals || [];

	const supportResistance =
	  equityData.supportResistance || [];

	const fundFlows =
	  equityData.fundFlows || [];
	const macroNumbers =
	  equityData.macroNumbers || [];

	const fundamentalSignals =
	  equityData.fundamentalSignals || [];
	  
	  if (
	  equityData.status === "loading"
	) {
	  ...
	  return (
		...
		<Panel>
		  ...
		  <div style={{
			...
			Equity signal engine unavailable
		  </div>
		  ...
		  />
						...
						<div style={{
						  ...
						  <div style={{
							...
							Live Technical Interpretation Engine
						  </div>

						  <Chip
	  label={
		equityData?.signals?.marketScore >= 60
		  ? "BULLISH BIAS"
		  : equityData?.signals?.marketScore <= 40
		  ? "BEARISH BIAS"
		  : "NEUTRAL BIAS"
	  }
	...
	/> 
				...
				columnGap:"8px",
						...
						marginBottom:6,
								...
								{equityData.technicalSignals.map((signal,i)=>(
								  <div key={i} style={{
									display:"grid",
									gridTemplateColumns:
				  isMobile
				...
				columnGap:"8px",
					...
					justifySelf:"start",
					  ...
					  <Chip label={signal.signal} bull={signal.bull}/>
					</div>
					</div>
								))}
							  </Panel>
						  ...
						  isMobile
							...
							: "26% 16% 58%",
								...
								{(equityData.supportResistance || []).map((level,i)=>(
								  <div key={i} style={{
									display:"grid",
						gridTemplateColumns:
						  isMobile
						...
						padding:"8px 0",
						  ...
						  {level.level}
									...
									<div style={{
									  fontFamily:"monospace",
									  color: level.level.includes("Support") ? "#00c97a" : "#ff4d6d"
									}}>{level.price}</div>
									<div style={{
						  color:"#a9c3dc",
						  fontSize:"0.68rem",
						  ...
						  padding:"16px 18px"
								...
								{fundamentalSignals.map((signal,i)=>(
	  <SignalBar
		key={i}
		name={signal.name}
		score={signal.score}
			...
			<div>
			  ...
			  <div style={{ marginBottom: 8 }}>
				...
				<div style={{
				  ...
				  <div style={{
					...
					Sector Rotation Heatmap & Momentum
				  </div>
			  ...
			  <div style={{
				...
				<div style={{
				  ...
				  Sector Performance & Signal Heatmap — MTD Returns + Relative Strength
				</div>
	  ...
	  overflow: "hidden"
			  ...
			  <div style={{
				...
				<div style={{
				  ...
				  <div style={{
					...
					right: 0,
					height: 1,
					background: "linear-gradient(90deg,transparent,rgba(0,212,170,0.10),transparent)"
				  }} />
					  ...
					  {sectorData.rotation.map((s, i) => (
						...
						<tr key={i}>
						  ...
						  <td style={{
							...
							color: "#dfeaf2",
							fontSize: "0.76rem",
							borderBottom: "1px solid rgba(30,45,61,0.28)",
							lineHeight: 1.18
						  }}>
							{s.sector}
						  </td>
							...
							<span style={{
							  ...
							  background:
								s.bull === true
								  ...
								  : "rgba(122,155,181,0.10)",
							  border:
								s.bull === true
								  ? "1px solid rgba(0,201,122,0.18)"
								  : s.bull === false
								  ? "1px solid rgba(255,77,109,0.18)"
								  : "1px solid rgba(122,155,181,0.16)",
							  color:
								s.bull === true
								  ? "#00c99a"
								  : s.bull === false
				...
				<div style={{
				  ...
				  <div style={{
					...
					Smart Money Sector Flow (MTD, ₹Cr)
				  </div>

				  {sectorData.flows.map(([name, value, bull], i) => {
				  ...
				  {/* THEMATIC */}
				  <div style={{
					borderTop: "1px solid rgba(30,45,61,0.55)",
					marginTop: 14,
					paddingTop: 14
				  }}>
								  ...
								  {[
									...
									<div key={i} style={{
									  ...
									  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
										<div style={{ fontSize:"0.78rem", fontWeight:800, color:"#eef4fa" }}>{name}</div>
							  ...
							  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
								<Panel>
								  ...
								  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.7rem" }}>
									...
									<tbody>
									  {(LIVE_EQUITY_CACHE.globalData || []).map((r,i)=>(
										<tr key={i}>
			...
			letterSpacing:"0.03em", color:s.color, letterSpacing:"0.08em", marginBottom:6 }}>{s.label}</div>
									  {s.lines.map((l,j)=>(
							  ...
							  <div style={{ display:"grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap:12 }}>
								<Panel>
								  ...
								  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:"0.7rem" }}>
									...
									<tbody>
									  {(LIVE_EQUITY_CACHE.oiSignals || []).map((r,i)=>(	
										<tr key={i}>
								...
								<Panel>
								  ...
								  {[
									...
									<div key={i} style={{ background:v.bg, border:`${v.active?"2px":"1px"} solid ${v.border}`, borderRadius:5, padding:12, marginBottom:10 }}>
									  <div style={{ fontSize:"0.6rem", fontWeight:800,
			letterSpacing:"0.03em", color:v.color, letterSpacing:"0.08em", marginBottom:4 }}>{v.range} — {v.label}</div>
			...
			letterSpacing:"0.03em", letterSpacing:"0.06em", textTransform:"uppercase",
							  ...
							  <div style={{ background:"#0d1219", border:"1px solid #1e2d3d", borderRadius:8, padding:"24px 28px" }}>
								<div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20 }}>
								  <div style={{ display:"flex", alignItems:"center", gap:10, fontSize:"0.65rem", fontWeight:800,
			letterSpacing:"0.03em", letterSpacing:"0.08em", textTransform:"uppercase", color:"#00d4aa", fontFamily:"monospace" }}>
			...
			letterSpacing:"0.03em", letterSpacing:"0.08em", textTransform:"uppercase",
								...
								{!narrative && !loading && !error && (
								  <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:128, color:"#3d5268", gap:8 }}>
			...
			letterSpacing:"0.03em", color:"#f5a623", letterSpacing:"0.08em", marginBottom:8 }}>API CONNECTION NOTE</div>
							  ...
							  <div style={{ display:"flex", gap:2, padding:"0 20px", background:"#0d1219", borderBottom:"1px solid #1e2d3d", overflowX:"auto" }}>
								{TABS.map(t=>(
								  <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{
									padding:"10px 16px", fontSize:"0.65rem", fontWeight:800,
			letterSpacing:"0.03em",
